...
function CommentTree({
	...
	async function toggleLike() {
		try {
			if (node.likedByMe) {
				await apiJson(`/api/comments/${node.id}/like`, {
					method: 'DELETE',
				});
			} else {
				await apiJson(`/api/comments/${node.id}/like`, {
...
export function PostCard({ post, me, onPostRefresh, onDelete }: Props) {
	...
	const reactCountLabel =
		post.likeCount > 9 ? '9+' : String(Math.max(0, post.likeCount));
								...
								<li className='_feed_timeline_dropdown_item'>
									<a
										...
										onClick={(e) => {
											...
											if (isAuthor)
												alert(
													'Edit post is not available yet.',
												);
										}}
									>
										<span>
