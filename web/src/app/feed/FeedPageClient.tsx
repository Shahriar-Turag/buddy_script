...
async function fetchPostsWithComments(search: string): Promise<{
  ...
  const data = await apiJson<{ posts: Post[]; nextCursor: string | null }>(`/api/posts${search}`);
...
export function FeedPageClient() {
  ...
  const loadInitial = useCallback(async () => {
    ...
    try {
      const { posts: nextPosts, nextCursor: nc } = await fetchPostsWithComments("?limit=20");
      setPosts(nextPosts);
      setNextCursor(nc);
    } catch (e) {
  ...
  const refreshSinglePost = useCallback(async (postId: string) => {
    setRefreshBanner(null);
    try {
      const data = await apiJson<{ post: Post }>(`/api/posts/${postId}/hydrate`);
      ...
      setRefreshBanner("Could not refresh this post. Reload the page if it looks out of date.");
    }
  }, []);
  ...
  async function loadMore() {
    ...
    setLoadMoreError(null);
    try {
      const { posts: more, nextCursor: nc } = await fetchPostsWithComments(
        `?limit=20&cursor=${encodeURIComponent(nextCursor)}`
      );
      setPosts((p) => [...p, ...more]);
      setNextCursor(nc);
    } catch (e) {
      setLoadMoreError(e instanceof ApiError ? e.message : "Could not load more posts");
    } finally {
              ...
              <div className="col-xl-6 col-lg-6 col-md-12 col-sm-12">
                <div className="_layout_middle_wrap">
                  <div className="_layout_middle_inner">
                    {error ? (
                      <p className="_social_login_content_para _mar_b16" style={{ color: "#c53030" }}>
                        {error}
                      </p>
                    ) : null}
                    {refreshBanner ? (
                      <p className="_social_login_content_para _mar_b16" style={{ color: "#b7791f" }}>
                        {refreshBanner}
                      </p>
                    ) : null}
                    {loadMoreError ? (
                    ...
                    />
                    {loading ? (
                      <p className="_social_login_content_para text-center _mar_b16">Loading feed…</p>
                    ) : null}
                    {posts.map((post) => (
                      <PostCard
                        key={post.id}
                        post={post}
                        me={me}
                        onPostRefresh={refreshSinglePost}
                        onDelete={removePost}
                      />
                    ))}
                    {nextCursor ? (
